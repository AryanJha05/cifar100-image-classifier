...
function setApplicationState(newState) {
  ...
  if (stateInvalid) stateInvalid.classList.add('hidden');
  ...
  switch (newState) {
    ...
    case STATES.LOADING:
      ...
      if (subhead) subhead.textContent = 'Inference in progress... Evaluating visual features and domain boundary.';
      break;
    ...
    case STATES.INVALID:
      ...
      if (subhead) subhead.textContent = 'Image analyzed. Visual features do not match the CIFAR-100 target domain.';
      break;
...
function resetAndCenterClassifier(smooth = true) {
  ...
  const invalidThumb = document.getElementById('invalid-preview-thumb');
  if (invalidThumb) invalidThumb.src = '';
...
async function executePrediction() {
  ...
  try {
    ...
    // Check OOD / Validity response
    if (result.valid === false) {
      const invalidThumb = document.getElementById('invalid-preview-thumb');
      if (invalidThumb && currentImageDataUrl) {
        invalidThumb.src = currentImageDataUrl;
      }
      const invalidMsg = document.getElementById('invalid-message-text');
      if (invalidMsg && result.message) {
        invalidMsg.textContent = result.message;
      }
      setApplicationState(STATES.INVALID);
    } else {
